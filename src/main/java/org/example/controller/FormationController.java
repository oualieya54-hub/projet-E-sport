...
public class FormationController {
    ...
    public Formation checkDuplicate(String jeu, int idCoach, java.time.LocalDate dateDebut, int excludeId) throws SQLException {
        return formationService.checkDuplicate(jeu, idCoach, dateDebut, excludeId);
    }
    ...
    void handleAdd(ActionEvent event) {
        if (validateInput()) {
            try {
                ...
                Formation duplicate = checkDuplicate(newFormation.getJeu(), newFormation.getIdCoach(),
                                                      newFormation.getDateDebut(), 0);
                if (duplicate != null) {
                    showAlert("Formation dupliquée", "Doublon détecté",
                        String.format("Une formation existe déjà pour:\n- Coach: %d\n- Jeu: %s\n- Date: %s",
                            newFormation.getIdCoach(), newFormation.getJeu(), newFormation.getDateDebut()),
    ...
    void handleUpdate(ActionEvent event) {
        if (!isEditMode) {
            ...
            if (selected == null) {
                showAlert("Aucune sélection", "Veuillez sélectionner une formation",
                    "Sélectionnez une formation dans la liste pour la modifier.", Alert.AlertType.WARNING);
                return;
            }
    ...
    private void enterEditMode(Formation formation) {
        isEditMode = true;
        ...
        showAlert("Mode édition", "Veuillez modifier les champs",
            "Vous êtes maintenant en mode édition.\nModifiez les champs et cliquez sur 'Enregistrer'.",
    ...
    private void saveFormation() {
        ...
        if (validateInput()) {
            try {
                currentFormation.setTitre(titreField.getText());
                currentFormation.setDescription(descArea.getText());
                currentFormation.setJeu(jeuCombo.getValue());
                currentFormation.setNiveau(niveauCombo.getValue());
                currentFormation.setDureeSemaines(Integer.parseInt(dureeField.getText()));
                currentFormation.setPrix(Float.parseFloat(prixField.getText()));
                currentFormation.setDateDebut(dateDebutPicker.getValue());
                currentFormation.setStatut(statutCombo.getValue());

                Formation duplicate = checkDuplicate(currentFormation.getJeu(), currentFormation.getIdCoach(),
                                                      currentFormation.getDateDebut(), currentFormation.getIdFormation());
                if (duplicate != null) {
                    showAlert("Formation dupliquée", "Doublon détecté",
                        String.format("Une autre formation existe déjà pour:\n- Coach: %d\n- Jeu: %s\n- Date: %s",
                            currentFormation.getIdCoach(), currentFormation.getJeu(), currentFormation.getDateDebut()),
                        Alert.AlertType.WARNING);
                    return;
                }

                this.update(currentFormation);
                loadData();
                exitEditMode();
